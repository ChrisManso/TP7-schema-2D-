...
void Mesh2D::BuildTrianglesCenter()
{
  ...
  for (size_t i = 0; i < _triangles.size(); i++) {
    ...
    u[0]=S2[0]-S1[0];u[1]=S2[1]-S1[1];
    v[0]=S3[0]-S1[0];v[1]=S3[1]-S1[1];

    _tri_area[i]=(u[0]*v[1]-v[0]*u[1])/2.;
    _tri_center[i][0]=(S1[0]+S2[0]+S3[0])/3.;
    _tri_center[i][1]=(S1[1]+S2[1]+S3[1])/3.;
...
void Mesh2D::BuildEdgesNormal()
{
  ...
  for (size_t i = 0; i < _edges.size(); i++) {
    int n1 = _triangles[i].GetEdge()(0);
    int n2 = _triangles[i].GetEdge()(1);
    double x1 = _vertices[n1](0);
    double y1 = _vertices[n1](1);
    double x2 = _vertices[n2](0);
    double y2 = _vertices[n2](1);
    ...
    _edg_center[i][0]=(S1[0]+S2[0])/2.
    _edg_center[i][1]=(S1[1]+S2[1])/2.
